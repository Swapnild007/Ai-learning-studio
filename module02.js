...
(() => {
  ...
  m.researchBasis = [
    ...
    {institution:"Stanford / Emory / Washington",course:"An Introduction to Statistical Learning with Python",focus:"regression, classification, resampling, regularization, nonlinear methods, trees, SVM and unsupervised learning",url:"https://www.statlearning.com/"},
    {institution:"MIT",course:"6.390 Introduction to Machine Learning",focus:"modeling/prediction, overfitting, generalization, regression, classification and clustering",url:"https://ocw.mit.edu/courses/res-tll-008-social-and-ethical-responsibilities-of-computing-serc/"}
  ];
  m.masteryGate = {
  ...
  const P = {
    "Regression & Classification":{
      ...
      vocab:["Feature = input variable.","Target = quantity being predicted.","Hypothesis = function selected from a hypothesis class.","Loss = numerical prediction error.","Empirical risk = average loss on observed data.","Regularization = constraint or penalty controlling model complexity.","Logit = real-valued score mapped to probability.","Calibration = agreement between predicted probabilities and observed frequencies."],
    ...
    "Trees & Ensembles":{
      ...
      vocab:["Impurity = measure of label mixing in a node.","Information gain = impurity reduction after a split.","Leaf = terminal tree region.","Pruning = restricting tree complexity.","Bagging = fitting models on bootstrap samples and aggregating them.","Random forest = bagged trees with feature randomness.","Boosting = sequentially adding learners focused on current errors.","Weak learner = model performing above a trivial baseline."],
    ...
    "Unsupervised Learning":{
      ...
      mental:["Clustering does not reveal objectively true groups without a supporting question.","K-means assumes Euclidean geometry through its objective.","GMM gives soft component membership under probabilistic assumptions.","PCA finds variance directions, not guaranteed semantic features."],
    ...
    "Evaluation & Validation":{
      ...
      vocab:["Training set = data used to fit parameters.","Validation set = data used for model choices.","Test set = held-out final assessment.","Cross-validation = repeated held-out fitting/evaluation.","Bootstrap = resampling with replacement to study variability.","Precision = fraction of predicted positives that are correct.","Recall = fraction of actual positives recovered.","Leakage = information crossing a boundary where it should not be available."],
      mental:["A test set is not a second validation set.","Metric choice encodes which errors matter.","Cross-validation reduces dependence on one split but does not make leakage safe.","Uncertainty belongs beside a point estimate when sampling variability matters."],
